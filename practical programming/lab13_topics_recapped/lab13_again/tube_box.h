...
public:
    TubeBox(string label="", int number=0, float radius=0.0f, float height=0.0f): Box(label, number), m_radius{radius}, m_height{height}
    {     }

    virtual ~TubeBox() {  }

    void setALL(float radius, float height) { m_radius = radius; m_height=height; }
